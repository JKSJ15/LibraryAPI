...
public class GlobalExceptionsHandler {
	...
	public ResponseEntity<ExceptionModel> useBookNotFoundException(BookNotFoundException e) {
		ExceptionModel model = new ExceptionModel(e.getMessage(), LocalDateTime.now(), HttpStatus.NOT_FOUND,
	...
	public ResponseEntity<ExceptionModel> useInvalidDateException(InvalidDateException e) {
		ExceptionModel model = new ExceptionModel(e.getMessage(), LocalDateTime.now(), HttpStatus.BAD_REQUEST,
	...
	public ResponseEntity<ExceptionModel> useUserAlreadyExistsException(UserAlreadyExistsException e) {
		ExceptionModel model = new ExceptionModel(e.getMessage(), LocalDateTime.now(), HttpStatus.CONFLICT,
	...
	public ResponseEntity<ExceptionModel> useInvalidTokenException(InvalidTokenException e) {
		ExceptionModel model = new ExceptionModel(e.getMessage(), LocalDateTime.now(), HttpStatus.UNAUTHORIZED,
	...
	public ResponseEntity<ExceptionModel> useMethodArgumentNotValidException(MethodArgumentNotValidException e) {
		ExceptionModel model = new ExceptionModel(e.getBindingResult().getFieldErrors().get(0).getDefaultMessage(),
				LocalDateTime.now(), HttpStatus.BAD_REQUEST, HttpStatus.BAD_REQUEST.value());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(model);
	}
	...
	public ResponseEntity<ExceptionModel> useConstraintViolationException(ConstraintViolationException e) {
		ExceptionModel model = new ExceptionModel(e.getConstraintViolations()
				...
				.findFirst()
				.get()
				.getMessage(), LocalDateTime.now(), HttpStatus.BAD_REQUEST, HttpStatus.BAD_REQUEST.value());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(model);
	}
